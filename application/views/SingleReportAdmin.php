	...
	<div class="container">
	    ...
	    <!--/.navbar-collapse-->
				...
				foreach ($report as $key) {
					?>
					<h3 class="radius"><?= $key['category']; ?></h3>
					<p><?= $key['description']; ?></p>
					<?php
				}
				?>
				...
				<div class="col-lg-5" style="text-align:center; margin-top:5%; margin-bottom:3%;">
					<table class="col_3 cool margin-bottom" width="100%">
						<th><h3 class="radius">Reported by</h3></th>
							...
							foreach ($reporter as $value) {
								...
								<tr><td class="padding">
									...
									base64_decode(base64_decode($value['employee_email']));
									?></a>
								</td></tr>
								<?php
							}
...
<tr><td>Sorry no result found.</td></tr>
					<?php
						...
						{
							foreach ($morereports as $value) {
								?>
								<tr><td class="padding">
									<a href="<?= base_url(); ?>admin/singleReport?id=<?= $value['id']?>&&client=recruiter&&reporter=<?= $value['uid']?>&&user=<?= $value['rid']; ?>"><?=
									$value['description'];
									?></a>
								</td></tr>
