						...
						<th><h3 class="radius">Reported by</h3></th>
							...
							foreach ($reporter as $value) {
								...
								<tr><td class="padding">
									...
									base64_decode(base64_decode($value['recruiter_email']));
									?></a>
								</td></tr>
								<?php
							}
					...
					<?php
						...
						else
						{
							foreach ($morereports as $value) {
								?>
								<tr><td class="padding">
									<a href="<?= base_url(); ?>admin/singleReport?id=<?= $value['id']; ?>&&client=employee&&reporter=<?= $value['rid']; ?>&&user=<?= $value['uid']; ?>"><?=
									$value['description'];
									?></a>
								</td></tr>
